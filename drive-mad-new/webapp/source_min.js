...
function touchMove(ev) {if (ev.target.closest?.('#mod-panel, #mod-toggle')) return;
    ev.preventDefault();
    for (let i = 0; i < ev.changedTouches.length; i++) {
        let t = ev.changedTouches.item(i);
        _touch_move(t.identifier, t.clientX, t.clientY);
...
function touchEnd(ev) {if (ev.target.closest?.('#mod-panel, #mod-toggle')) return;
    ev.preventDefault();
    for (let i = 0; i < ev.changedTouches.length; i++) {
        let t = ev.changedTouches.item(i);
...
function mouseMove(ev) {if (ev.target.closest?.('#mod-panel, #mod-toggle')) return;
    ev.preventDefault();
    _mouse_move(ev.button, ev.clientX, ev.clientY);
}

function mouseUp(ev) {if (ev.target.closest?.('#mod-panel, #mod-toggle')) return;
    ev.preventDefault();
